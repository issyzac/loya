...
import PromotionalMessagesContainer from './components/promotional-messages/promotional-messages-container';
import { Link } from 'react-router-dom';

import logo from './assets/hze-logo.png';
import { useUser, useUpdateUser } from './providers/UserProvider';
import { useCurrentPage, useCustomerOrder, useProductToBuy, useUpdateCurrentPage, useUpdateRemovedItems } from './providers/AppProvider';
import Shop from './pages/shop';
import ProductDetails from './pages/product_details';
import OrderSummary from './elements/order-summary';
import { useState, useRef } from 'react';

import CustomerWalletPage from './pages/customer-wallet-page';
...
export default function Home() {
  ...
  const { clearCartAndRemovedItems } = useUpdateRemovedItems();
  ...
  const getRightPanel = () => {
    if ((productToBuy && currentPage === 'Product') || customerOrder.length > 0) {
      return <OrderSummary />;
    } else {
          ...
          <div className="relative flex items-center justify-center py-5 lg:justify-between">
            ...
            <div className="absolute right-0 shrink-0 lg:hidden pr-2 pt-8">
              ...
              <PopoverButton className="group relative inline-flex items-center justify-center rounded-md bg-transparent p-2 text-indigo-200 hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-white">
                ...
                <FontAwesomeIcon icon={faTimes} className="hidden size-6 group-data-[open]:block" />
              </PopoverButton>
            </div>
          </div>
          <NavigationMenu />
          <div className="mx-auto max-w-3xl pt-8 lg:pt-2 px-4 sm:px-6 lg:max-w-7xl">
            <div className="py-2">
            ...
            <div className="divide-y divide-gray-200 rounded-lg bg-white shadow-lg ring-1 ring-black/5">
              <div className="pb-2 pt-3">
                <div className="flex items-center justify-between px-4">
                  ...
                  <div className="-mr-2">
                    <PopoverButton ref={closeButtonRef} className="relative inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
                      <span className="absolute -inset-0.5" />
                ...
                <div className="mt-3 space-y-1 px-2">
                  <Button onClick={() => {
                    clearCartAndRemovedItems();
                    updateCurrentPage("Home");
                    closeButtonRef.current?.click();
                  }} className='block rounded-md px-3 py-2 text-base font-medium text-gray-900 hover:bg-gray-100 hover:text-gray-800' >
                    <span className="flex items-center">
                      <FontAwesomeIcon icon={faHouse} className="fa-fw mr-2" />
                      Home
                    </span>
                  </Button>
                  <Button onClick={() => {
                    ...
                    <span className="flex items-center">
                      <FontAwesomeIcon icon={faWallet} className="fa-fw mr-2" />
                      Wallet
                    </span>
                  </Button>
